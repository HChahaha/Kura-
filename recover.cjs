...
// the original part of the code BEFORE the modal started:
const startOfModalIdx = code.lastIndexOf('<div className="px-6 py-5 flex items-start justify-between">', idx);

if (startOfModalIdx !== -1) {
  // Now we need the part of the code AFTER the modal:
  const endOfModalAnchor = '{/* Part 3: Historical References Database */}';
  const lastEndOfModalIdx = code.lastIndexOf(endOfModalAnchor);
  
  if (lastEndOfModalIdx !== -1) {
     const cleanCodeBefore = code.substring(0, startOfModalIdx);
     const cleanCodeAfter = code.substring(lastEndOfModalIdx);
     
     // Let's insert the EXACT newModal using function replace.
  ...
  `;
      ...
      const res = cleanCodeBefore + newModal + cleanCodeAfter;
      fs.writeFileSync('src/pages/ShoppingList.tsx', res);
