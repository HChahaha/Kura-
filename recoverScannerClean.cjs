...
const t = code.split('const handleFinish =')[0]; // The broken part at top

const importsObj = \`import React, { useState, useEffect, useRef } from 'react';
...
const tMid = code.split('const handleFinish =')[0]; 
const rest = '\\n  const handleFinish =' + code.split('const handleFinish =').slice(1).join('const handleFinish =');

// Oh wait, did my old replace affect the second chunk of parse logic inside handleCameraCapture as well? Yes it did!
// I'll just rewrite the whole file from top until handleCameraCapture ENDS, and replace all that!

const finalCode = importsObj + rest;
fs.writeFileSync('src/pages/Scanner.tsx', finalCode);
