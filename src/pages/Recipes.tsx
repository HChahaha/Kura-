...
function RecipeGridCard(props: any) {
 ...
 <div 
 onClick={onClick}
 className="bg-white border border-zinc-200 rounded-2xl group cursor-pointer transition-all hover:border-forest-green relative hover:shadow-md flex flex-col h-[280px] shadow-sm overflow-hidden"
 >
 <div className="h-28 w-full bg-zinc-50 border-b border-zinc-100 flex-shrink-0 relative overflow-hidden flex items-center justify-center">
 {recipe.image ? (
 ...
 <div className="w-full h-full flex items-center justify-center bg-zinc-100 text-3xl group-hover:scale-110 transition-transform duration-500">
 {foodEmoji}
 </div>
 )}
 <div className="absolute top-2 right-2 z-10 flex gap-2">
 {recipe.isAiSuggested && (
 <span className="bg-forest-green text-white text-[9px] tracking-widest font-bold px-2 py-1 rounded shadow-sm">
 AI
 </span>
 )}
 <button 
 ...
 className="w-7 h-7 bg-white/90 hover:bg-white text-zinc-400 hover:text-red-500 rounded-full flex items-center justify-center shadow-sm transition-all"
 title="Delete recipe"
 >
 ...
 <h3 className="text-sm font-bold text-ink-black leading-tight line-clamp-2 mt-1">
 {recipe.name}
 </h3>
 <p className="text-[10px] text-zinc-500 font-medium mt-1.5 flex-1">
 {recipe.category || 'Kitchen Notebook'}
 </p>

 <div className="flex items-center justify-between text-[11px] font-semibold text-zinc-400 border-t border-zinc-100 pt-3 mt-auto">
